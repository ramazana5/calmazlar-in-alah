...
await (member.roles.add(role.id))
 message.channel.send(new Emran.MessageEmbed().setDescription(`${member} isimli üyeye \`${role.name}\` isimli yetki başarıyla verildi!`)  .setFooter('Bu komutu kullanan yetkili ' + message.author.tag, message.author.avatarURL).setColor('BLUE'));
    
  } catch (e) {
    console.log(e);
    message.channel.send('Vereceğin rolün üstüne koy ve yetkilerimin olduğundan emin ol');
  }
  //ZING
};
